...
public class GentleDataExtractor {
	...
	private List<Part> getWorkRelations(final List<ExtendedRelationship> relations) {
		...
		relations.stream()
			.map(ExtendedRelationship::getTo)
			.map(e -> new Part(e)).forEach(p -> {
				// Add creators names
				Set<ExtendedRelationship> partOf = filterSet(hatGeschaffen, 
						x -> x.getTo().getId().equals(p.getId()));
				...
				// Add location names
				List<ExtendedRelationship> locRels = filterList(befindetSichIn,
						x -> x.getFrom().getId().equals(p.getId()));
				Set<String> locs = locRels.stream()
						.map(ExtendedRelationship::getTo)
						.map(l -> getInstitution(l))
	...
	private Set<ExtendedRelationship> getRelationships(final File importFile) throws InterruptedException, ExecutionException, FileNotFoundException, JAXBException {
		...
		StreamSource streamSource = new StreamSource(new BufferedInputStream(new FileInputStream(new File(importFile.getAbsolutePath()))));
	...
	private Literature getLiterature(final Entity entity) {
		...
		Set<ExtendedRelationship> media = filterSet(literaturEnthaeltBilddatei,
				x -> x.getFrom().getId().equals(entity.getId()));
		List<String> mediums = media.stream()
				.map(ExtendedRelationship::getTo)
				.map(Entity::getId)
				.collect(Collectors.toList());
	...
	public List<Medium> getMediums(final String workId) {
		...
		Set<ExtendedRelationship> mediumSet = filterSet(bilddateiZuWerk, 
				x -> x.getTo().getId().equals(workId));
		List<Medium> mediums = new ArrayList<>();
		for (ExtendedRelationship exRel : mediumSet) {
			if (exRel.getFrom().getImagePath() == null)
				continue;
			Entity medium = exRel.getFrom();
	...
	private Person getPerson(final Entity entity, int depth) {
		...
		List<ExtendedRelationship> geborenIn = filterList(geburtsortVon, 
				x -> x.getTo().getId().equals(entity.getId()));
		...
		List<ExtendedRelationship> gestorbenIn = filterList(sterbeOrt, 
				x -> x.getTo().getId().equals(entity.getId()));
